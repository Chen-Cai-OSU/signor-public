...
def to_img(x):
    x = 0.5 * (x + 1)
    x = x.clamp(0, 1)
    x = x.view(x.size(0), 1, img_w, img_w)
    return x
...
img_transform = transforms.Compose([
    ...
    ReshapeTransform((-1, img_w * img_w))
])

dataset = CIFAR10('./data_', transform=img_transform, download=True)
...
class autoencoder(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Linear(64, 128),
            nn.ReLU(True), nn.Linear(128, img_w * img_w), nn.Tanh())
...
for epoch in range(num_epochs):
    ...
    if epoch % 10 == 0:
        ...
        save_image(pic, './cifar10_img/image_{}.png'.format(epoch))
    del img, loss
