...
__all__ = ['flatten', 'flatten2', 'concat_shape', 'broadcast', 'add_dim', 'add_dim_as_except', 'repeat', 'repeat_times',
           'force_view']
...
if __name__ == '__main__':
    tsr = torch.rand((100, 2, 3))

    assert flatten(tsr).size() == torch.Size([600])

    assert flatten2(tsr).size() == torch.Size([100, 6])

    print(add_dim(tsr, 1, 5).size())  # pt.Size([100, 5, 2, 3])
    print(add_dim(tsr, 1, 10).size())  # pt.Size([100, 10, 2, 3])
    print(add_dim(tsr, 3, 5).size())  # pt.Size([100, 2, 3, 5])
